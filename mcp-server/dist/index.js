...
server.tool("query", "Run a read-only SQL query", {
    ...
    try {
        ...
        const filtered = rows.filter(row => Object.values(row).every(value => value !== null));
        return {
            content: [
                {
                    type: "text",
...
server.tool("summarize_table", "Generate summary statistics for a table: numeric min/avg/max/count and categorical value counts", { table: z.string().describe("The table name to summarize") }, async ({ table }) => {
    ...
    try {
        ...
        const statsClauses = numericCols
            .map((col) => `MIN("${col}") AS "${col}_min", MAX("${col}") AS "${col}_max", AVG("${col}") AS "${col}_avg", COUNT("${col}") AS "${col}_count"`)
            .join(", ");
        const { rows: statRows } = await client.query(`SELECT ${statsClauses}
             FROM public."${table}";`);
        ...
        if (categoricalCols.length > 0) {
            const catResults = await Promise.all(categoricalCols.map(async (col) => {
...
server.tool("list_tables", "List all tables in the public schema", {}, // no user input
async () => {
      ...
      `);
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify(rows.map(r => r.table_name), null, 2),
                },
            ],
            isError: false,
        };
...
server.tool("describe_table", "Describe the columns of a table", { table: z.string().describe("The table name to inspect") }, async ({ table }) => {
